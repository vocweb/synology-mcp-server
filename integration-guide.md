...
```bash
# Remove synology mcp (local scope)
claude mcp remove synology -s local

# Remove synology mcp (user scope)
claude mcp remove synology -s user
```
...
**Remove MCP:** Delete the `synology` entry from the `mcpServers` object in `claude_desktop_config.json` and restart Claude Desktop.
...
**Remove MCP:** In **Cursor Settings → MCP**, click the trash icon next to `synology`, or delete the `synology` entry from `~/.cursor/mcp.json`. Reload Cursor.
...
**Remove MCP:** Toggle the server off under **Cascade → MCP servers**, then delete the `synology` entry from `~/.codeium/windsurf/mcp_config.json`. Reload Windsurf.
...
**Remove MCP:** Delete the `[mcp_servers.synology]` and `[mcp_servers.synology.env]` blocks from `~/.codex/config.toml` and restart `codex`.
...
**Remove MCP:**
- **Desktop MCP beta:** Remove the `synology` entry from the same JSON config and restart ChatGPT Desktop.
- **Custom GPT bridge:** In ChatGPT → **Configure → Actions**, delete the imported Synology action. Then stop the bridge process and revoke `${MCP_TOKEN}` on the server side.
...
**Remove MCP:** Remove the `BasicMCPClient` / `McpToolSpec` instances and rebuild `FunctionAgent` without the Synology tools. Optionally `pip uninstall llama-index-tools-mcp` if unused elsewhere.
